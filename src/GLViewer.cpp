...
void GLViewer::init(int argc, char **argv, 
    ...
    for (int i=0;i<camera_parameters.size();i++) {
        int sn = configurations[i].serial_number;
        frustums[ sn ] = createFrustum(camera_parameters[i]);
        PointCloud pointCloud;
        pointClouds_[ sn ] = pointCloud;
    }
    for (auto& it: frustums) {
        it.second.pushToGPU();
    }
    ...
    // Point cloud
    for (auto &it: pointClouds_)
        it.second.initialize(sl::Resolution(1280,720));
...
void GLViewer::updateData(std::map<int,sl::Transform>& camera_poses,
                          sl::Objects& camera_object,
                          sl::Bodies& camera_body,
                          std::map<int,sl::Mat>& camera_pointClouds)
{
    mtx.lock();
    objectsName.clear();
    BBox_edges.clear();
    BBox_faces.clear();
    skeletons.clear();
    for (auto& sk: skeletons_raw)
        sk.second.clear();
    fusionStats.clear();
    

    // Camera Objects
    for (auto &obj : camera_object.object_list) {
        if (obj.tracking_state == sl::OBJECT_TRACKING_STATE::OK) {
            auto clr_id = generateColorID(obj.id);
            createBboxRendering(obj.head_bounding_box, clr_id);         
        }
    }
    ...
    // Camera Point Clouds
    // for (auto &it: pointClouds_)
    //     it.second.pushNewPC(camera_pointClouds[it.first]);
    
    // Camera Poses
    int count(0);
    for (auto &it: camera_poses) {
        cam_poses[it.first] = it.second;
        frustums[count++].setRT(it.second);
        sl::float3 pos = it.second.getTranslation();
        createIDRendering(pos, sl::float4(1.f, 1.f, 0.f, 1.0f), it.first);
    }
    
    // Fused Bodies
    // for(auto &it:fused_bodies.body_list) {            
    //     auto clr_id = sl::float4(1,0,0,0.5);
    //     addSKeleton(it, skeletons, clr_id, false, sl::BODY_FORMAT::BODY_34);
    //     createIDRendering(it.position, clr_id, it.id);
    //     BBox.addBoundingBox(it.bounding_box, clr_id);
    // }

    // Camera Bodies

    for (auto &sk : camera_body.body_list) {
        auto& sk_r = skeletons_raw[sk.id];
        sk_r.setDrawingType(GL_QUADS);
        sk_r.clear();
        if(renderBody(sk, camera_body.is_tracked))
        {
            auto clr_id = generateColorID(sk.id);
            addSKeleton(sk, skeletons, clr_id, false, sl::BODY_FORMAT::BODY_34);
            // addSKeleton(sk, sk_r, clr_id, true);
        }            
    }   
    

    //    for (auto &it : camera_bodies) {
    //     auto clr_id = generateColorID(id++);
    //     // if (it.second.is_new) 
    //     // {
    //         auto& sk_r = skeletons_raw[it.first];
    //         sk_r.clear();
    //         sk_r.setDrawingType(GL_QUADS);
    ...
    //                 addSKeleton(sk, sk_r, clr_id, true);
    //                 // createBboxRendering(sk.bounding_box, sl::float4(1,1,1,0.5));
    //         }
    //     // }
            
    //     ObjectClassName obj_str;
    //     obj_str.name_lineA = "CAM: " + std::to_string(it.first);
...
void GLViewer::updateData(std::map<int,sl::Transform>& camera_poses,
                          std::map<int, sl::Objects>& camera_objects,
                          std::map<int, sl::Bodies>& camera_bodies,
                          std::map<int,sl::Mat>& camera_pointClouds)
{
    mtx.lock();
    objectsName.clear();
...
void GLViewer::update() {
    ...
    if (keyStates_['f'] == KEY_STATE::UP)
        currentInstance_->fused = !currentInstance_->fused;
...
void GLViewer::draw() {
    ...
    for (auto& it: frustums) {
        glUniformMatrix4fv(shaderLine.MVP_Mat, 1, GL_TRUE, (vpMatrix * cam_poses[it.first] ).m );
    ...
    // point cloud
    for (auto &it: pointClouds_) {
        it.second.draw(vpMatrix * cam_poses[it.first]);
        // it.second.draw(vpMatrix);
    }
